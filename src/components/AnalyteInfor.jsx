...
const AnalyteInfor = () => {
		...
		const fetchAnalytes = async () => {
			try {
				...
				const data = response.data.map((analyte) => ({
					...analyte,
					tat_expected: `${analyte.tat_expected.days} ${analyte.tat_expected.days > 1 ? 'days' : 'day'}`,
				}));
				setAnalytes(data);
	...
	const handleProtocolSelect = (index, protocol) => {
		// handleInputChange(index, 'protocol_code', protocol.protocol_code);
		// handleInputChange(index, 'protocol_id', protocol.id);
				...
				<div className="rounded-lg border p-0.5 relative z-0 overflow-x-auto">
					...
					<table className="min-w-screen-xl bg-white ">
						<thead className="border-b-2">
							<tr>
								<th className="py-2 text-center w-32">Mã chỉ tiêu</th>
								...
								<tr className="border-t bg-blue-50">
										...
										{newAnalyte.matrix === 'Khác' && (
											<input
												...
												onChange={(e) => {
													...
													handleNewAnalyteChange('matrix', e.target.value);
												}}
											/>
										)}
									</td>
									<td className="p-1 text-start">
										...
										{isProtocolDropdownVisible && (
											<div className="absolute top-full mt-1 w-full bg-white border rounded shadow-lg z-10">
												{protocols
													.filter((protocol) => protocol.protocol_code.includes(protocolSearch))
													.slice(0, 5)
													.map((protocol, index) => (
														<div
															key={index}
									...
									<td className="p-1 text-start">
										<input
											...
											value={newAnalyte.tat_expected.split(' ')[0]}
										...
										{editingRow === index ? (
											<>
												<input
													type="number"
													className="w-14 border px-2 py-1 rounded bg-white"
													value={analyte.tat_expected.split(' ')[0]}
